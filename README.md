...
项目是一个单文件网页应用，适合部署在 GitHub Pages，也可以添加到 iPhone 主屏幕，当作轻量级 PWA 使用。
...
```text
index.html                  主应用，包含 HTML、CSS、JavaScript、数据和逻辑
...
- 项目刻意保持单文件结构，所以主要逻辑和内容都在 `index.html` 中。
...
The project is a single-file web app designed for GitHub Pages. It can also be added to an iPhone home screen as a lightweight PWA.
...
There is no backend, no analytics, and the birthday is never sent to a server.
...
```text
index.html                  Main app: HTML, CSS, JavaScript, data, and logic
...
- The project intentionally stays self-contained, so most logic and content live in `index.html`.
